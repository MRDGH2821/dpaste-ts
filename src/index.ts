	...
	try {
		if (isValidSize(content) && isValidSyntax(syntax) && isValidExpiry(expiry_days)) {
			...
			if(isValidSize(content)){
				throw "Maximum allowed content size is 250,000 bytes";
			}
			else if(isValidSyntax(syntax) ){
				throw "Syntax must be from https://dpaste.com/api/v2/syntax-choices/"
			}
			else if(isValidExpiry(expiry_days)){
				throw "Expiry days must be greater than 1 day & not exceed 365 days"
			}
